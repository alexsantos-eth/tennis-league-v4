...
export const useNewMatchStore = create<NewMatchState>()(
  persist(
    (set, get) => ({
      ...
      resetInvitedPlayers: () => {
        set((state) => ({
          invitedPlayers: state.invitedPlayers[0]
            ? [state.invitedPlayers[0]]
            : [],
        }));
      },
      resetNewMatchStore: () => {
      ...
      toggleInvitedPlayer: (player) => {
        ...
        set((state) => {
          ...
          const hostPlayer = state.invitedPlayers[0];
          const guestPlayers = state.invitedPlayers.slice(1);
          const maxGuestInvites = getMaxGuestInvitesByMatchType(
            state.matchType,
          );
          ...
          const nextGuestPlayers = [
            ...guestPlayers,
            mapUserToMatchPlayer(player),
          ];
      ...
      handleSubmit: async (event) => {
        ...
        const invitedPlayersWithAssignments = guestPlayers.map(
          (player, index) => ({
            ...player,
